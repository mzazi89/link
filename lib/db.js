 ...
 * applied. The raw driver message — `relation "device_credentials" does not
 * exist` — names the symptom but not the fix, and it is exactly the sort of thing
 * that gets misread as a code bug. Rewrite it into something actionable before it
 * reaches a log or an operator's screen.
 *
 * Note this only ever fires for the tables THIS site owns. A missing
 * `bot_control` or `bot_status` means the bot has not started, which is a
 * different problem with a different fix, and is reported as such.
 */
...
// Only the tables this site owns. bot_control and bot_status are quartz's and
// are deliberately absent: if they are missing the bot has not been started, and
// creating them here would be inventing a schema another service has to live
// with.
export const EXPECTED_TABLES = ['device_credentials', 'password_attempts', 'request_log']
...
// ── Waking the bot ───────────────────────────────────────────────────────────
//
// ── OPTIONAL SPEED PATH — currently inert ────────────────────────────────────
//
// Requests reach the bot by polling `bot_control`, which quartzxd's README puts
// at roughly 15 seconds. That is the floor on how long a user waits before
// anything happens.
//
// This is the escape hatch. The bot_control consumer in quartz is a poll loop;
// adding a LISTEN on this channel to it turns that 15s into milliseconds, with
// no change on this side. Until someone does that, these notifications are sent
// and dropped on the floor — quartz is not listening, so they cost one tiny
// statement and change nothing.
//
// It is called anyway so the seam already exists and the improvement is a
// three-line change in the bot rather than a redesign here. Do not mistake it
// for something that is currently making pickup fast: nothing is.
export const NOTIFY_CHANNEL = process.env.NOTIFY_CHANNEL || 'mzazi_bot_requests'

/**
 * Signal that a request is waiting. Call only AFTER the row is committed, or a
 * listener could wake and find nothing.
 *
 * Never throws, and never awaited for correctness — a notification is an
 * optimisation and the bot's own poll is the real mechanism.
 */
export async function notifyRequest(requestId) {
  try {
    await getPool().query('SELECT pg_notify($1, $2)', [NOTIFY_CHANNEL, String(requestId)])
  } catch (err) {
    console.warn(
      '[link][db] could not notify the bot — its own poll will still find it:',
      err.message
    )
  }
}
...
export async function schemaExists() {
  // Keyed on the first table this site owns, not on the queue it used to have.
  // Checking a table that is no longer created would answer "missing" forever
  // and leave the bootstrap path permanently open.
