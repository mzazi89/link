...
// ── Waking the bot ───────────────────────────────────────────────────────────
//
// Polling alone puts a floor on how fast a pairing code can appear: the user
// waits, on average, half a poll interval before a worker even looks at their
// request. Postgres NOTIFY removes that floor — the site signals the channel the
// moment a row is committed and the bot drains immediately.
//
// The poll is not removed, it is demoted to a safety net: if the bot's LISTEN
// connection drops, or the notification is missed, the next poll still finds the
// row. Speed on the happy path, no correctness dependency on a side channel.
export const NOTIFY_CHANNEL = process.env.NOTIFY_CHANNEL || 'mzazi_device_requests'

/**
 * Signal that a request is waiting. Call only AFTER the row is committed, or the
 * bot can wake and find nothing.
 *
 * Never throws. A notification is an optimisation; failing the user's request
 * because a hint could not be delivered would be the wrong trade when the
 * fallback poll is still there.
 */
export async function notifyRequest(publicId) {
  try {
    await getPool().query('SELECT pg_notify($1, $2)', [NOTIFY_CHANNEL, publicId])
  } catch (err) {
    console.warn(
      '[link][db] could not notify the bot — the poll fallback will pick it up:',
      err.message
    )
  }
}
