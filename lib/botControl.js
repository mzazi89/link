...
export async function createPairRequest(phone, passwordHash) {
  const { rows } = await query(
    `
 ...
 */
export async function createUnpairRequest(phone) {
  const { rows } = await query(
    `
    INSERT INTO bot_control (action, payload, status)
    VALUES ($1, $2::jsonb, 'pending')
    RETURNING id, created_at
    `,
    [ACTION_UNPAIR, JSON.stringify({ number: phone, mode: 'delete', source: 'link' })]
  )
  return rows[0]
}
